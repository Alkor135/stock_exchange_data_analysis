...
import sqlighter3_RTS_day_futures_options
...
def get_future_date_results(tradedate: date, tiker: str):
    ...
    with requests.Session() as session:
        ...
        while tradedate != today_date:
            ...
            if not sqlighter3_RTS_day_futures_options.tradedate_futures_exists(connection, cursor, tradedate):
                ...
                if len(df) != 0:  # Если полученный ответ не нулевой, чтобы исключить выходные дни
                    ...
                    if len(df) == 1:  # Если одна строка в DF
                        if not df['OPEN'].isnull().values.any():  # Проверка на пустые значения поля 'OPEN'
                            # Запись строки в БД
                            sqlighter3_RTS_day_futures_options.add_tradedate_future(
                                connection, cursor, df.loc[0]['TRADEDATE'],
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    if sqlighter3_RTS_day_futures_options.non_empty_table_futures(connection, cursor):
        # Меняем стартовую дату на дату последней записи
        start_date = datetime.strptime(sqlighter3_RTS_day_futures_options.get_max_date_futures(connection, cursor),
                                       "%Y-%m-%d").date()
