    ...
    with requests.Session() as session:
        ...
        while tradedate != today_date:
            ...
            if len(df) != 0:  # Если полученный ответ не пустой, чтобы исключить выходные дни
                ...
                if df.shape[0] != 0:  # Строка с нужным инструментом есть на дату
                    # df = df.drop(['BOARDID', 'SHORTNAME', 'NAME', 'VALUE', 'DURATION', 'YIELD', 'DECIMALS',
                    #               'CAPITALIZATION', 'CURRENCYID', 'DIVISOR', 'TRADINGSESSION', 'VOLUME'], axis=1)
