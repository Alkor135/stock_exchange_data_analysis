"""
Создание БД с таблицей дневных котировок индекса MCFTR (индекс полной доходности MOEX)
При доступе из других модулей получает доступ к БД.
"""
from pathlib import Path
from typing import Any
import sqlite3

import pandas as pd


def create_tables(connection, cursor):
    """ Функция создания таблиц  в БД если их нет"""
    try:
        with connection:
            # cursor.execute('''DROP TABLE Day''')
            # print("Удалена таблица 'Day' из БД")
            cursor.execute('''CREATE TABLE if not exists Day (
                            TRADEDATE         DATE PRIMARY KEY UNIQUE NOT NULL,
                            SECID             TEXT NOT NULL,
                            OPEN              REAL NOT NULL,
                            LOW               REAL NOT NULL,
                            HIGH              REAL NOT NULL,
                            CLOSE             REAL NOT NULL)'''
                           )
        print('Taблицы в БД созданы')
    except sqlite3.OperationalError as exception:
        print(f"Ошибка при создании БД: {exception}")


def non_empty_table(connection, cursor):
    """Проверяем, есть ли записи в таблице 'Day' в БД"""
    with connection:
        return cursor.execute("SELECT count(*) FROM (select 1 from Day limit 1)").fetchall()[0][0]


def tradedate_exists(connection, cursor, tradedate):
    """Проверяем, есть ли указанная дата в таблице 'Day' в БД"""
    with connection:
        result = cursor.execute('SELECT * FROM `Day` WHERE `TRADEDATE` = ?', (tradedate,)).fetchall()
        return bool(len(result))


def add_tradedate(connection, cursor, tradedate, secid, open, low, high, close):
    """Добавляет строку в таблицу Day """
    with connection:
        return cursor.execute(
            "INSERT INTO `Day` (`TRADEDATE`, `SECID`, `OPEN`, `LOW`, `HIGH`, `CLOSE`) VALUES(?,?,?,?,?,?)",
            (tradedate, secid, open, low, high, close))


def get_tradedate_future(connection):  # Используется для перебора дат в
    """ Возвращает Dataframe с: дата торгов, короткое имя, последний день торгов из БД SQL по фьючерсам """
    with connection:
        return pd.read_sql('SELECT `TRADEDATE`, `SHORTNAME`, `LSTTRADE` FROM `Day`', connection)


def get_tradedate_future_date(connection, cursor, datedraw):
    """ Возвращает: дата торгов, low, high, close, короткое имя, последний день торгов из БД SQL на дату """
    with connection:
        return cursor.execute('SELECT LOW, HIGH, CLOSE '
                              'FROM Day WHERE TRADEDATE = ?', (datedraw,)).fetchall()[0]


def get_max_date(connection, cursor):
    """ Получение максимальной даты по индексу """
    with connection:
        return cursor.execute('SELECT MAX (TRADEDATE) FROM Day').fetchall()[0][0]


if __name__ == '__main__':  # Создание БД, если её не существует
    # Настройка базы данных
    tiker: str = 'MCFTR'
    path_bd: Path = Path(r'c:\Users\Alkor\gd\data_quote_db')  # Папка с БД
    file_bd: str = f'{tiker}_index_day.db'

    if not path_bd.is_dir():  # Если не существует папка под БД
        try:
            path_bd.mkdir()  # Создание папки под БД
        except Exception as err:
            print(f'Ошибка создания каталога "{path_bd}": {err}')

    connection: Any = sqlite3.connect(fr'{path_bd}\{file_bd}', check_same_thread=True)
    cursor: Any = connection.cursor()
    create_tables(connection, cursor)  # Создание таблиц в БД если их нет
